...
const PokeCard = ({ pokeInfo }) => {
  ...
  return (
    <div className="container">
      <article
        className={`card border-${pokemon?.types[0].type.name}`}
              ...
              <ul className="card__list">
                {pokemon?.stats.map((statInfo) => (
                  <li className="card__stat" key={statInfo.stat.url}>
                    <span className="card__stat__label">
                      {statInfo.stat.name}
                    </span>
                    <span
                      className={`card__stat_value color-${pokemon?.types[0].type.name}`}
                    >
                      {statInfo.base_stat}
                    </span>
                  </li>
